...
// Dados fallback para uso em caso de erro
const fallbackData: ParceiroData = {
...
async function scrapeVakinha(): Promise<ParceiroData> {
  try {
    ...
    return { ...fallbackData, fetchedAt: new Date().toISOString() };
  }
}

export async function getParceiroData(): Promise<ParceiroData> {
  const now = Date.now();
...
export function getFallbackData(): ParceiroData {
  return { ...fallbackData, fetchedAt: new Date().toISOString() };
}
