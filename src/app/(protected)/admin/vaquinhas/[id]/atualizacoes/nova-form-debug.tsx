...
export function NovaAtualizacaoFormDebug({ vaquinhaId }: { vaquinhaId: string }) {
  ...
  useEffect(() => {
    const imagesInfo = form.imagens.length > 0
      ? `${form.imagens.length} images: ${form.imagens.map(img => img.id.substring(0, 6)).join(", ")}`
      : "No images";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("FORM SUBMISSION - Current state:", JSON.stringify(form, null, 2));
      console.log(`Form has ${form.imagens.length} images`);

      if (form.imagens.length === 0 && form.tipo === "GALERIA") {
        console.error("VALIDATION ERROR: No images in gallery");
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-4">
      ...
      {/* Debug Info Panel */}
      <div className="p-3 bg-blue-900/30 border border-blue-500 rounded-lg text-xs font-mono whitespace-pre-line">
        <h3 className="text-blue-400 font-bold mb-1">Debug Info:</h3>
        <p className="text-blue-300">{debugInfo}</p>
      </div>
      ...
      <Button
        ...
        className="w-full"
      >
        {galleryUploading ? 'Aguarde o upload das imagens...' : 'Publicar Atualização (Debug)'}
      </Button>
    </form>
  );
}