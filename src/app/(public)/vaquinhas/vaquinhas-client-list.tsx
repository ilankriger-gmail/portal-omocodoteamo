    ...
    <div className="px-4 py-8">
      ...
      <div className="mb-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-zinc-900/50 border border-zinc-800 rounded-lg text-white text-xs placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-green-500"
          />
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-[10px] font-medium transition-colors whitespace-nowrap ${
            !statusFilter
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-[10px] font-medium transition-colors flex items-center gap-1 whitespace-nowrap ${
            statusFilter === "ATIVA"
              ? "bg-green-600 text-white"
              : "bg-zinc-800 text-zinc-300 hover:bg-zinc-700"
          }`}
                  ...
                  return (
                    ...
                    >
                      <div className="aspect-[4/3] relative flex-shrink-0">
                        ...
                        <div className="absolute top-2 right-2 px-1.5 py-0.5 rounded-full text-[10px] font-medium bg-green-500/20 text-green-400">
                          Ativa
                        </div>
                        {v.videoUrl && (
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-2">
                          <h3 className="text-white text-xs font-semibold line-clamp-2 min-h-[2.5rem]">{v.titulo}</h3>
                        </div>
                      </div>
                      <div className="p-2 flex flex-col flex-grow">
                        <div className="flex justify-between text-[10px] mb-1 min-h-[1rem]">
                        ...
                        <div className="mb-2 flex-grow">
                          <div className="flex items-center justify-between text-[10px] mb-1">
                            ...
                            <span className="text-zinc-500">R$ {v.meta.toLocaleString("pt-BR")}</span>
                          </div>
                          <div className="h-1 bg-zinc-800 rounded-full overflow-hidden">
                            <div className="h-full rounded-full bg-gradient-to-r from-green-500 to-emerald-400" style={{ width: `${progress}%` }} />
                          </div>
                          <div className="text-center text-[10px] text-zinc-500 mt-0.5">{Math.round(progress)}%</div>
                        </div>
                        <div className="w-full py-1.5 bg-green-600 hover:bg-green-500 text-white text-[10px] font-medium rounded-lg text-center transition-colors mt-auto">
                          Quero Ajudar
                        </div>
                      </div>
                    </Link>
                  ...
                  return (
                    ...
                    >
                      <div className="aspect-[4/3] relative flex-shrink-0">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
                        <div className="absolute top-2 right-2 px-1.5 py-0.5 rounded-full text-[10px] font-medium bg-zinc-500/20 text-zinc-400">
                          Encerrada
                        </div>
                        {v.videoUrl && (
                      ...
                      <div className="p-2 flex flex-col flex-grow">
                        ...
                        <div className="mb-2 flex-grow">
                          <div className="flex items-center justify-between text-[10px] mb-1">
                            <span className="text-zinc-400 font-semibold">R$ {v.valorAtual.toLocaleString("pt-BR")}</span>
                            <span className="text-zinc-500">R$ {v.meta.toLocaleString("pt-BR")}</span>
                          </div>
                          <div className="h-1 bg-zinc-800 rounded-full overflow-hidden">
                        ...
                        <div className="w-full py-1.5 bg-zinc-700 hover:bg-zinc-600 text-white text-[10px] font-medium rounded-lg text-center transition-colors mt-auto">
                          Ver Detalhes
                        </div>
                      </div>
                    </Link>
                  ...
                  >
                    <div className="aspect-[4/3] relative flex-shrink-0">
                      ...
                      <div className={`absolute top-2 right-2 px-1.5 py-0.5 rounded-full text-[10px] font-medium ${
                        v.status === "ATIVA" ? "bg-green-500/20 text-green-400" : "bg-zinc-500/20 text-zinc-400"
                      }`}>
                    ...
                    <div className="p-2 flex flex-col flex-grow">
                      <div className="flex justify-between text-[10px] mb-1 min-h-[1rem]">
                        <span className={v.status === "ATIVA" ? "text-green-400" : "text-zinc-400"}>
                          {v.atualizacoesCount > 0 ? `${v.atualizacoesCount} atualização${v.atualizacoesCount !== 1 ? "ões" : ""}` : ""}
                        </span>
                        <div className="flex items-center gap-2">
                          {v.doacoes > 0 &&
                            <span className={v.status === "ATIVA" ? "text-green-400 flex items-center gap-0.5" : "text-zinc-400 flex items-center gap-0.5"}>
                              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" />
                              </svg>
                              {v.doacoes}
                          ...
                          {v.coracoes > 0 &&
                            <span className={v.status === "ATIVA" ? "text-red-400 flex items-center gap-0.5" : "text-zinc-400 flex items-center gap-0.5"}>
                              <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" stroke="none">
                                <path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3.3.67-4.47 2.7C10.85 3.67 9.3 3 7.5 3A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z" />
                              </svg>
                              {v.coracoes}
